...
vstack tiling, butter filter, more conv+bn, zoom aug, other cv, full tiling
"""
...
class HMSModel(nn.Module):
    ...
    def forward_image_feats(self, img, specs):
        ...
        bs, _, h, w = img.shape
        img = (
            img.reshape(bs, 2, 2, 4, h, w)
            .permute((0, 2, 3, 4, 1, 5))
            .reshape(bs, 1, h * 8, w * 2)
        )
        specs = (
            specs.reshape(bs, 2, 2, 4, h, w)
            .permute((0, 2, 3, 4, 1, 5))
            .reshape(bs, 1, h * 8, w * 2)
        )
...
class HMSLightningModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        if self.step % max(self.trainer.accumulate_grad_batches // 2, 1) == 0:
...
def main(args):
    ...
    # overlap_df = pd.merge(overlap_df, patient_cv, on="patient_id")
