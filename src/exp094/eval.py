...
import torch
from main import BASE_DIR, EXP_ID, HMSDataModule, HMSLightningModel2_5D
...
def main(args):
    ...
    for fold, (train_idx, valid_idx) in enumerate(
        ...
        model = (
            HMSLightningModel2_5D.load_from_checkpoint(
    ...
    for i in range(len(cv_fold)):
        print(f"CV fold{i}:", cv_fold[i])
    print(
        "CV mean:",
        F.kl_div(
            torch.log(torch.tensor(preds_all)),
