...
vstack tiling, butter filter, more conv+bn, zoom aug, using all data(val: vote > 7), low vote mean
"""
...
class HMSDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        try:
            ...
            middle = 25
        signals = get_signals(eeg_id, middle)
        signals = np.stack(
            [
        ...
        try:
            ...
            if self.train and np.random.rand() < 0.5:
                signals = signals.reshape(4, 4, -1)
                signals = signals[np.random.permutation(4)].reshape(16, -1)

            if self.train and np.random.rand() < 0.5:
                specs = specs[np.random.permutation(4)]
...
class HMSModel(nn.Module):
    def __init__(
        ...
        #     [
        #         nn.Sequential(
        #             nn.Conv2d(num_features, 512, kernel_size=1),
    ...
    def forward_image_feats(self, img, specs):
        ...
        bs, _, h, w = img.shape
        img = (
            img.reshape(bs, 4, 4, h, w)
            .permute((0, 2, 1, 3, 4))
            .reshape(bs, 4, h * 4, w)
        )
        specs = (
            specs.reshape(bs, 4, 4, h, w)
            .permute((0, 2, 1, 3, 4))
            .reshape(bs, 4, h * 4, w)
        )
    ...
    def forward_head(self, img_feats):
        # output = self.head[0](img_feats)
        # output_votes = self.head[1](img_feats)
        # return output, output_votes
...
class HMSLightningModel(pl.LightningModule):
    def __init__(
        self,
        height: int = 128,
        width: int = 256,
    ...
    def calc_loss(self, outputs: torch.Tensor, labels: torch.Tensor):
        ...
        #     outputs["logits_votes"],
        #     labels["targets_votes"].to(dtype=outputs["logits"].dtype),
        # )
        losses["loss"] = losses["kldiv"]  # + losses["bce"]
        return losses
    ...
    def training_step(self, batch, batch_idx):
        ...
        signals, labels, specs, votes_type = (
            batch["signals"],
            batch["labels"],
            batch["specs"],
            batch["votes_type"],
        ...
        if (
            ...
            # votes_type = self.mixupper.lam * votes_type + (
            #     1 - self.mixupper.lam
            # ) * votes_type.flip(0)
        else:
            pass
        image = self.pipeline(signals)
        # outputs["logits"], outputs["logits_votes"] = self.model(image, specs)
        outputs["logits"] = self.model(image, specs)

        loss_target["targets"] = labels
        # loss_target["targets_votes"] = labels
    ...
    def add_model_specific_args(
        ...
        parser = parent_parser.add_argument_group("HMSLightningModel")
        parser.add_argument(
            "--height",
            default=128,
            type=int,
...
def main(args):
    ...
    for fold, (train_idx, valid_idx) in enumerate(
        ...
        new_overlap = (
            overlap_df.loc[:, ["eeg_id"] + labels_column].groupby("eeg_id").mean()
        )
        overlap_df = overlap_df.drop(labels_column, axis="columns")
        overlap_df = pd.merge(overlap_df, new_overlap, on="eeg_id")

        train_df_tmp = overlap_df.groupby("eeg_id").first().reset_index()
        # 全体の中からvalidに使われているpatient_idを除外する
        train_df = []
        for i in range(len(train_df_tmp)):
            if train_df_tmp.iloc[i].patient_id not in valid_df.patient_id.values:
                train_df.append(train_df_tmp.iloc[i])
        train_df = pd.concat(train_df, axis=1).T.reset_index(drop=True)
        ...
        early_stopping = callbacks.EarlyStopping(
            monitor="val_loss", patience=5, log_rank_zero_only=True
        )
        trainer = pl.Trainer(
