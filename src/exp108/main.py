...
vstack tiling, butter filter, more conv+bn, zoom aug, finetune from exp107
"""
...
def main(args):
    ...
    for fold, (train_idx, valid_idx) in enumerate(
        ...
        resume_dir = f"../../logs/{args.resume_dir}/fold{fold}"
        print(f"resume_dir: {resume_dir}")
        model = HMSLightningModel.load_from_checkpoint(
            glob(f"{resume_dir}/**/best_loss.ckpt", recursive=True)[0],
            pretrained=False,
            height=args.height,
            width=args.width,
