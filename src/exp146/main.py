...
vstack tiling, butter filter, more conv+bn, zoom aug, 2.5D(shuffle -> flip), fix cv, full tiling
"""
...
class HMSModel2_5D(nn.Module):
    ...
    def forward_image_feats(self, img, specs):
        ...
        bs, _, h, w = img.shape
        img = img.reshape(bs * 4, 1, 4 * h, w)
        specs = specs.reshape(bs * 4, 1, 4 * h, w)
        img = self.specaug_eeg(img)
        specs = self.specaug_kaggle(specs)

        img = torch.concat([img, specs], -1)  # (bs * zdim, 4, h * 4, w * 2)
